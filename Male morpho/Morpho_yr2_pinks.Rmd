...
---
01/14/22
Added Github connection

01/13/22
30 min of morning work

01/12/22
Morning work on MANCOVA (it's all signficant!) and setting up the basic linear morpho analysis

01/06/22
Removed duplicates and added missing fish


Last touched:01/04/22
Fixed outlier 73

02/11/22 - restarted this file to hopfully fix a github connection issue


01/03/22


Version to use for Y1 data: Morpho112221

In this version:
We're just editing the last years data to fit with the NEW morpho data from 2021
Things we're doing:
-add in morpho 2021 data
- and use the cubic (rather than quadratic) unbending as an addtional dataframe
- will explore morpho measuremnents for the 2021 males as well
THE WORK:
...
#setwd("/Users/alexandrareich/Desktop/MORPHO IN ACTION/WORKDIR")
#root.dir("/Users/alexandrareich/Desktop/MORPHO IN ACTION/WORKDIR")
```
...
Load my tps file
```{r}
p2.lands <- readland.tps("/Users/alexandrareich/Desktop/MORPHO IN ACTION/pink 2021 tps files_updated_010522/Files to use for analysis_clarified_010522/pink2021_landmarks.fixed73.deleteextras.appendhere_010522.tps copy", readcurves=T, specID= "imageID")


#need to order this goddamn chaos above. Need to add in hatch or wild. DO THIS NEXT!!!


p2.lands.unbent.cubic <- readland.tps("/Users/alexandrareich/Desktop/MORPHO IN ACTION/pink 2021 tps files_updated_010522/Files to use for analysis_clarified_010522/pink2021_cub_010522.tps copy") #no curves, they're all landmarks here. Oh shit, I didn't delete the excess landmakrs. I'll have to go do that. #landmakrs 18 thru 23 need to go because they were just used for unbending.



#I don't think I'll use the quadratic unbending. It did not seem to work as well as the cubic
p2.lands.unbent.quad <- readland.tps("/Users/alexandrareich/Desktop/MORPHO IN ACTION/pink 2021 tps files_updated_010522/Files to use for analysis_clarified_010522/pink2021_quad_010522.tps copy") #this might be wrong. Too few landmakrs. Maybe I didn't properly update in tpsdig.. Oh wait, it looks ok! Just need to delete the excess landmakrs (18 through 23)
...
plotAllSpecimens(p2.lands.cub)
##wow! Unbending makes a HUGE differece!!!

#what does unbent quadratic look like?
plotAllSpecimens(p2.lands.quad)
#fuck, these are exactly the same.
##I bet I fucked up in structing them in tps Util.
##they are either both cubic or both quadratic.
##I'll have to go to the bootcamp windows side of my computer and see what went wrong
##I'll probably
...
```{r}
pinkcurves <- as.matrix(read.csv("/Users/alexandrareich/Desktop/MORPHO IN ACTION/curveslide.csv", header=T))
...
```{r}
wild.or.hatch <- read.csv("/Users/alexandrareich/Desktop/MORPHO IN ACTION/pink2021.wildorhatch.andorder.csv")
```
...
IDEA: what if I remove hatchery snakey fish? (there were four of them)
(Which ones were the snakes?)
(the ones with reall small dpeths)
```{r}
...
p_2_old <- read.csv("/Users/alexandrareich/Desktop/THESIS TIME!/Compilation of GITHUB project code/Morpho_Code_040522/Data/Male.p2.Rdata.2_alt_ID.csv") #not working...why?
names(p_2_old)
p_2_old$ID
df.pink2021.orig$ID
...
```{r}
p2.males <- read.csv("/Users/alexandrareich/Desktop/THESIS TIME!/Field Data Year 2/Fish/Male.p2.Rdata.2.csv")
```
...
p2.males_dateadj<- read.csv("p2.males2_dateadj.csv")
