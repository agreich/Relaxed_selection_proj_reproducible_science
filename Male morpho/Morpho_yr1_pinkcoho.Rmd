...
gpa_coho_long <- gpagen(coho_lands_long, curves=pinkcurves)
#plotAllSpecimens(coho_lands_long)
plot(gpa_coho_long)
...
Because I traced curves, I may need semilandmarks. I also may need to revise these traced curves in TPS DIG, so that I don't resample. This either requires going back and drawing curves, or more semilandmarks
...
p.dat <- read.csv("/Users/alexandrareich/Desktop/Masters Proposal/R_code_Git_WD/MASTERMalePinksQC.reorder.csv")
#names(p.dat)
...
residual plots of models
```{r}
#####coho depth (long fish)
#residual vs. fitted plot
res <- resid(model.aov.cD)
plot(fitted(model.aov.cD), res) 
abline(0,0)
...
#######coho snout (long fish)
res <- resid(mod.c.snout)
plot(fitted(mod.c.snout), res)
abline(0,0)
#qq plot
qqnorm(res)
qqline(res)
...
#maybe test for normality? meh, qq is pretty solid
...
res <- resid(mod.p.d.l)
plot(fitted.values(mod.p.d.l), res)
abline(0,0)
#qq plot
qqnorm(res)
qqline(res)
...
RW correlations: 10/13/21 (can't remember why I wanted to do this)
See undergraduate student guide to geometric morphometrics
See if a RW is correlated with body size (Csize)... but why?
```{r}
...
Edits 10/25/21
#need to graph pinks without the outlier
```{r}
...
Time for a length comparison graph
```{r}
