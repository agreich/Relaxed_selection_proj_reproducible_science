---
title: "Cleaned up analyses, QCed_cohosub"
author: "Alex"
date: "11/20/2021"
...
library(here)
p1 <- read.csv(here::here("..","..","DATA","FemalePinkGSIotodataadd1_without_gaps copy.csv"))
...
#sample sizes
length(p1.wild.GSI.ttest$GSI) #10
...
p2.GSI <- read.csv(here::here("..","..","DATA", "Female.p2.Rdata.3.csv"))
names(p2.GSI)
...
p2_GSI_clean_date_alt <- read.csv(here::here("..","..","DATA","p2.GSI.clean_altdate.csv")) 
...
#sample sizes
length(GSI.for.ttest.hatch$GSI.1)
length(GSI.for.ttest.wild$GSI.1)
...
COHO year 1: GSI SUBSETTED
```{r}
c.GSI.dat <- read.csv(here::here("..","..","DATA","MASTERFemaleCohoQCwitheggs_copy.csv"))
...
#edits 10
#c.GSI.clean <-559 to 608
c.GSI.clean <- c.GSI.clean %>% filter(Length..mm.>= 559&Length..mm.<=608)
...
#t = 1.4608, df = 51.395, p-value = 0.07507 -> old
#t = 1.4608, df = 52, p-value = 0.07504 -> old
#new: t = 2.0034, df = 37, p-value = 0.02625

sd(c.GSI.wild.t.test$GSI)
sd(c.GSI.hatch.t.test$GSI)

#get the sample sizes for GSI
length(c.GSI.wild.t.test$GSI) 
length(c.GSI.hatch.t.test$GSI) 
...
#anova try it 11/05/22  
c_GSI_anova <- lm(GSI ~ Wild.or.Hatch + Length..mm., c.GSI.clean)
summary(c_GSI_anova) #length is not sig in subsetted data.
...
P1 Eggs
```{r}
p1.all.eggs <- read.csv(here::here("..", "..", "DATA", "P1.egg.diameters.csv"))
names(p1.all.eggs)
head(p1.all.eggs)

#the other pink even data
p1.other.data <- read.csv(here::here("..","..", "DATA", "FemalePinkGSIotodataadd1_without_gaps copy.csv"))
...
#sample sizes
unique(p1.df.clean$ID)

p1_hatchcount_eggs <- p1.df.clean %>% filter(Otolith.results=="PORT ARMSTRONG")
length(unique(p1_hatchcount_eggs$ID))

p1_wildcount_eggs <- p1.df.clean %>% filter(Otolith.results=="no mark")
length(unique(p1_wildcount_eggs$ID))
...
#pink odd (p2)###########################
p2.all.eggs <- read.csv(here::here("..", "..", "DATA", "Results.p2.ImageJ.eggs.csv"))
library(tidyverse)
...
#sample sizes
hatch_sample_size_p2egg <- p2.df.clean %>% filter(Oto.reading.4 == "PORT ARMSTRONG")
hatch_ss_p2egg <- length(unique(hatch_sample_size_p2egg$ID)) #43
wild_sample_size_p2egg <- p2.df.clean %>% filter(Oto.reading.4 == "No Mark")
wild_ss_p2egg <- length(unique(wild_sample_size_p2egg$ID)) #34
...
#c.all.eggs <- read.csv("coho.egg.diameters.csv")
#c.all.other.stuff <- read.csv("MASTERFemaleCohoQCwitheggs_copy.csv")
c.all.eggs<-read.csv(here::here("..", "..", "DATA", "coho.egg.diameters.csv"))
c.all.other.stuff <- read.csv(here::here("..", "..", "DATA", "MASTERFemaleCohoQCwitheggs_copy.csv"))
...
coho.clean <- coho.clean %>% filter(Length..mm. >= 559 & Length..mm. <=608) 

#number of hatch and wild in new (sub) dataset calc
hatch_count_eggs <- coho.clean%>% filter(Wild.or.Hatch=="hatchery")
wild_count_eggs <- coho.clean%>% filter(Wild.or.Hatch=="wild")
length(unique(hatch_count_eggs$Fish.ID)) #22
length(unique(wild_count_eggs$Fish.ID)) #18 (includes WF-25)
...
##make a csv for the GSI results
NamesGSI <- as.vector(c("GSI mean hatch", "GSI mean wild", "SD hatch GSI", "SD wild GSI", "Sample size hatch", "Sample size wild", "t GSI", "p-val GSI", "Degrees of freedom,"))
estimate_p1_ttest <-  p1.GSI.t.test$estimate
sd_p1_GSI_h <- sd(p1.hatch.GSI.ttest$GSI)
sd_p1_GSI_w <- sd(p1.wild.GSI.ttest$GSI)
ss_hatch_p1_GSI <- length(p1.hatch.GSI.ttest$GSI)
ss_wild_p1_GSI <- length(p1.wild.GSI.ttest$GSI)
t_GSI_p1 <- p1.GSI.t.test$statistic
p_GSI_p1 <- p1.GSI.t.test$p.value
df_GSI_p1 <- p1.GSI.t.test$parameter

p1_results_GSI <- c(estimate_p1_ttest[1],estimate_p1_ttest[2], sd_p1_GSI_h, sd_p1_GSI_w , ss_hatch_p1_GSI, ss_wild_p1_GSI, t_GSI_p1 , p_GSI_p1, df_GSI_p1)

tib_base <- as_tibble(NamesGSI)
p1_results_GSI_tibble <- cbind(NamesGSI, p1_results_GSI)
write.csv(p1_results_GSI_tibble, "Pink odd-year GSI results.csv")

##
estimate_p2_ttest <-  p2.GSI.t.test$estimate
sd_p2_GSI_h <- sd(GSI.for.ttest.hatch$GSI.1)
sd_p2_GSI_w <- sd(GSI.for.ttest.wild$GSI.1)
ss_hatch_p2_GSI <- length(GSI.for.ttest.hatch$GSI.1)
ss_wild_p2_GSI <- length(GSI.for.ttest.wild$GSI.1)
t_GSI_p2 <- p2.GSI.t.test$statistic
p_GSI_p2 <- p2.GSI.t.test$p.value
df_GSI_p2 <- p2.GSI.t.test$parameter

p2_results_GSI <- c(estimate_p2_ttest[1],estimate_p2_ttest[2], sd_p2_GSI_h, sd_p2_GSI_w , ss_hatch_p2_GSI, ss_wild_p2_GSI, t_GSI_p2 , p_GSI_p2, df_GSI_p2)

p2_results_GSI_tibble <- cbind(NamesGSI, p2_results_GSI)
write.csv(p2_results_GSI_tibble, "Pink even-year GSI results.csv")
##
estimate_coho_ttest <-  coho.GSI.t.test$estimate
sd_coho_GSI_h <- sd(c.GSI.hatch.t.test$GSI)
sd_coho_GSI_w <- sd(c.GSI.wild.t.test$GSI)
ss_hatch_coho_GSI <- length(c.GSI.hatch.t.test$GSI)
ss_wild_coho_GSI <- length(c.GSI.wild.t.test$GSI)
t_GSI_coho <- coho.GSI.t.test$statistic
p_GSI_coho <- coho.GSI.t.test$p.value
df_GSI_coho <- coho.GSI.t.test$parameter

coho_results_GSI <- c(estimate_coho_ttest[1],estimate_coho_ttest[2], sd_coho_GSI_h, sd_coho_GSI_w , ss_hatch_coho_GSI, ss_wild_coho_GSI, t_GSI_coho , p_GSI_coho, df_GSI_coho)

coho_results_GSI_tibble <- cbind(NamesGSI, p2_results_GSI)
write.csv(coho_results_GSI_tibble, "Coho GSI results.csv")
...
names_eggs_p1 <- c("Mean egg size hatch", "Mean egg size wild", "t egg", "p-val egg", "degrees of freedom", "Among fish SD", "Within fish SD") #"SD hatch egg", "SD wild egg",
mean_egg_hatch_p1 <- 6.681267  #from fixef(fit.p1.lme.param.noint)
mean_egg_wild_p1 <- 6.801640 #from fixef(fit.p1.lme.param.noint)
t_p1_egg <- 1.424 #from summary(fit.p1.C1.REML)
p_p1_egg <- 0.162/2 #from summary(fit.p1.C1.REML) and divided by 2 because 1-sided t test
df_p1_egg <- 44 #from summary(fit.p1.C1.REML)
among_fish_sd_pink <- 0.233099 #both this and below from: 
within_fish_sd_pink <- 0.2839699 ##nlme::intervals(fit.p1.lme.param.yesint)

results_p1 <- c(mean_egg_hatch_p1 , mean_egg_wild_p1, t_p1_egg , p_p1_egg, df_p1_egg, among_fish_sd_pink, within_fish_sd_pink)

pink_results_egg<- cbind(names_eggs_p1, results_p1)
write.csv(pink_results_egg, "Even year pink results eggs.csv")
...
names_eggs_p2 <- c("Mean egg size hatch", "Mean egg size wild", "t egg", "p-val egg", "degrees of freedom", "Among fish SD", "Within fish SD")
mean_egg_p2_h <- 6.414625
mean_egg_p2_w <- 6.194072 #based on fixef above
t_egg_p2 <- 3.843 #based on summary(fit.p2.B4.reml)
p_egg_p2 <- 1 #based on summary(fit.p2.B4.reml)
df_p2 <- 75 #based on summary(fit.p2.B4.reml)
among_fish_sd_p2 <- 0.2411312 #from output above
within_fish_sd_p2 <- 0.1637388

results_p2_egg <- c(mean_egg_p2_h, mean_egg_p2_w, t_egg_p2, p_egg_p2, df_p2, among_fish_sd_p2, within_fish_sd_p2)

p2_egg_results <- cbind(names_eggs_p2,results_p2_egg )
write.csv(p2_egg_results, "Odd year pink egg results.csv")
...
fixef(fit.lme.no.int)
summary(fit.lme.no.int)
summary(fit.c.C1.REML) #t value from here!! and p-value
summary(fit.lme)
anova(fit.lme, test=T, type="marginal")
(anova_coho_egg<-anova(fit.c.C1.REML, test=T, type="marginal"))
##length is sig here, isnt it. Don't I need to report estimates with length in the formula then...? 05/24/22
#maybe...

#t test: 0.000531/2 = 0.0002655 #IS THE OLD
#t-test sub: 0.001044/2= 0.000522 #this is new t-test

#automate csv output coho eggs
names_eggs <- c("Mean egg size hatch", "Mean egg size wild",  "Sample size hatch", "Sample size wild", "t egg", "p-val egg", "degrees of freedom", "Among fish SD", "Within fish SD") #"SD hatch egg", "SD wild egg",
sum_coho <- summary(fit.lme.no.int)
#sd_hatch_egg <- 0.07369 
#sd_wild_egg <- 0.10984
temp_sd_w <- coho.clean %>% filter(Wild.or.Hatch=="wild")
sd(temp_sd_w$Diameter..mm.)

SS_coho_egg_h <- 22
SS_coho_egg_w <- 18
t_egg_coho <- 3.551 #taken from summary(fit.c.C1.REML)
p_egg_coho <- 0.001044/2 # taken from summary(fit.c.C1.REML) and then divided by 2 because one-sided
df_cohoegg<- 38 #also taken from summary(fit.c.C1.REML)
among_fish_sd_coho <- 0.3430652  
within_fish_sd_coho <- sum_coho$sigma #taken from the sum_coho output


coho_egg_results <- c(fixef(fit.lme.no.int)[1],fixef(fit.lme.no.int)[2], SS_coho_egg_h, SS_coho_egg_w, t_egg_coho, p_egg_coho, df_cohoegg, among_fish_sd_coho, within_fish_sd_coho)

coho_egg_results_df <- cbind(names_eggs, coho_egg_results)
write.csv(coho_egg_results_df,"Coho egg results")
...
#hatch
  ...
  coord_cartesian(ylim=c(5,8.3), xlim=c(558.5,610)) +
  scale_y_continuous(breaks=c(5,6,7,8), expand=c(0,0))+
...
ggplot(c.GSI.clean) + aes(x=Length..mm., y=GSI, color=Wild.or.Hatch) + geom_point(size=3) +
  ...
  guides(color="none") +
  labs(x="Length (mm)") +
  coord_cartesian(ylim=c(13,30), xlim=c(559.5,610)) +scale_x_continuous (expand=c(0,0))+
...
ggGSI_coho <- ggplot(c.GSI.clean) + aes(x=Length..mm., y=GSI, color=Wild.or.Hatch) + geom_point(size=3) +
  ...
  guides(color="none") +
  labs(x="Length (mm)") +
  coord_cartesian(ylim=c(13,30), xlim=c(558.5,610)) +scale_x_continuous (breaks=c(500,550,600,650), expand=c(0,0))+
  scale_y_continuous(breaks=c(15, 20, 25, 30), expand=c(0,0)) #+
  #theme(plot.margin = margin(t=7,r=12,l=5,b=5))
...
ggGSI_coho <- ggplot(c.GSI.clean) + aes(x=Length..mm., y=GSI, color=Wild.or.Hatch) + geom_point(size=3) +
  ...
  guides(color="none") +
  labs(x="Length (mm)") +
  coord_cartesian(ylim=c(13,30), xlim=c(558,610)) +scale_x_continuous (expand=c(0,0))+
  scale_y_continuous(breaks=c(15, 20, 25, 30), expand=c(0,0)) #+
  #theme(plot.margin = margin(t=7,r=12,l=5,b=5))
...
ggsave ("GSI_Eggs_combined1_cohosub.jpg", width = dev.size()[1], height = dev.size()[2]); dev.off()

```
Lenght test, in the subsetted coho world
```{r}
c.GSI.dat_2 <- c.GSI.dat %>% filter(Length..mm.>= 559&Length..mm.<=608)
c.GSI.clean_length <- c.GSI.dat_2 %>% filter (Weird == "n")
...
sd(c.GSI_hatch_Ltest$Length..mm.)
sd(c.GSI_wild_Ltest$Length..mm.)
